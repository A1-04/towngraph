...
/**
 * An in-memory, reference implementation of the property graph interfaces
...
public class TinkerGraph implements IndexableGraph, KeyIndexableGraph, Serializable {
	...
	public Edge addEdge(final Object id, final Vertex outVertex, final Vertex inVertex, final String label) {
		...
		if (null != id) {
			...
			/*
			 * if (null != edge) { throw ExceptionFactory.edgeWithIdAlreadyExist(id); }
			 * non-necessary in our case, because our IDs are irrelevant
			 */
		} else {
			boolean done = false;
			while (!done) {
	...
	private String getNextId() {
		...
		while (true) {
			...
			this.currentId++;
			if (null == this.vertices.get(idString) || null == this.edges.get(idString)
	...
	protected class TinkerKeyIndex<T extends TinkerElement> extends TinkerIndex<T> implements Serializable {
		...
		public void createKeyIndex(final String key) {
			...
			if (TinkerVertex.class.equals(this.indexClass)) {
				KeyIndexableGraphHelper.reIndexElements(graph, graph.getVertices(),
						new HashSet<String>(Arrays.asList(key)));
			} else {
				KeyIndexableGraphHelper.reIndexElements(graph, graph.getEdges(),
						new HashSet<String>(Arrays.asList(key)));
			}
		}
